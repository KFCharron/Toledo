...
public class PacingReportWriter {
    ...
    public static Workbook writeReport(ArrayList<PacingAdvertiser> ads, Set<String> flightNames) {
        ...
        Sheet troubleSheet = wb.createSheet("Watch List");
        ...
        int rowCount = 1;
        ArrayList<String> troubleFlightNames = new ArrayList<>();
        for (String flightName : flightNames) {

            DateTime flightStart = new DateTime().plusYears(20);
            DateTime flightEnd = new DateTime(1);
            ...
            for (PacingAdvertiser a : ads) {
                for (PacingLineItem l : a.getLineList()) {
                    ...
                    if (lineFlightName.equals(flightName)) {
                        ...
                        if (l.getStartDate().isBefore(flightStart)) flightStart = l.getStartDate();
                        if (l.getEndDate().isAfter(flightEnd)) flightEnd = l.getEndDate();
            ...
            if (((totalRow.getCell(3).getNumericCellValue() - .20d) > totalRow.getCell(4).getNumericCellValue()) ||
                    (((totalRow.getCell(3).getNumericCellValue() - .10d) > totalRow.getCell(4).getNumericCellValue())
                            && daysLeft <= 7))
                troubleFlightNames.add(flightName);


        }
        for (Cell c : header) summarySheet.autoSizeColumn(c.getColumnIndex());

        // Trouble Sheet headers,
        // For each in trouble sheet...........
        header = troubleSheet.createRow(0);
        header.createCell(0).setCellValue("Line Item");
        header.createCell(1).setCellValue("Start Date");
        header.createCell(2).setCellValue("End Date");
        header.createCell(3).setCellValue("% Time Through Flight");
        ...
        for (String flightName : troubleFlightNames) {
            ...
            // Header per advertiser
            Row advertHeader = troubleSheet.createRow(rowCount++);
            advertHeader.createCell(0).setCellValue(flightName);
            advertHeader.getCell(0).setCellStyle(adHeaderStyle);
            for (int x = 1; x < 13; x++) {
                advertHeader.createCell(x);
            ...
            for (PacingAdvertiser a : ads) {
                for (PacingLineItem l : a.getLineList()) {
                    ...
                    if (lineFlightName.equals(flightName)) {
                        ...
                        if (l.getStartDate().isBefore(flightStart)) flightStart = l.getStartDate();
                        if (l.getEndDate().isAfter(flightEnd)) flightEnd = l.getEndDate();
                        Row dataRow = troubleSheet.createRow(rowCount++);
                        dataRow.createCell(0).setCellValue(l.getName());
                        dataRow.createCell(1).setCellValue(l.getStartDate().toString("MMM-dd"));
                        dataRow.createCell(2).setCellValue(l.getEndDate().toString("MMM-dd"));
        ...
        for (Cell c : header) troubleSheet.autoSizeColumn(c.getColumnIndex());
