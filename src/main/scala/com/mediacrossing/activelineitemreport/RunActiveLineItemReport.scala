...
object RunActiveLineItemReport extends App {
  ...
  headRow.createCell(1).setCellValue("Status")
  headRow.createCell(2).setCellValue("Start Date")
  headRow.createCell(3).setCellValue("End Date")
  ...
  lis.foreach(l => {
    ...
    d.createCell(0).setCellValue(l.name)
    d.createCell(1).setCellValue(l.status)
    d.createCell(2).setCellValue(start.getMonthOfYear + "/" + start.getDayOfMonth + "/" + start.getYear)
    d.createCell(3).setCellValue(end.getMonthOfYear + "/" + end.getDayOfMonth + "/" + end.getYear)
    if (end.isBefore(new DateTime().plusWeeks(1))) {
      d.getCell(0).setCellStyle(red)
      d.getCell(1).setCellStyle(red)
      d.getCell(2).setCellStyle(red)
      d.getCell(3).setCellStyle(red)
    }
    rowCount += 1
  })
